...
Description:
    ...
    Then the http instance will be added to a work queue, one of other work threads will process the request which instance carries.
    After a work thread acquires the instance, processRead will be called to read and parse the http message.
    Finally, processRequest will be called to generate response, which is then written to send buffer and sent to client.
*/
